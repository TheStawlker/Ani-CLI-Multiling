#!/bin/sh
# Installateur ani-cli pour macOS
# Usage: ./install-macos.sh [--system]
...
command -v mpv >/dev/null || command -v vlc >/dev/null || \
    command -v iina >/dev/null || \
    echo "Attention: aucun lecteur détecté (installez mpv: brew install mpv)."
