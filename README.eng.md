...
> 🇫🇷 **Documentation complète en français : [README.fr.md](README.fr.md)** (sources VF/VOSTFR anime-sama & franime, site par défaut, installateurs)
...
* Can I watch dub? - Yes, use `--dub` (French preferred by default, English fallback).
* Can I change dub language? - Yes, use `--dub-lang` with a comma separated list of preferred languages, tried in order per episode. Example: `ani-cli --dub-lang fre,eng one piece`. Codes or names are accepted (`fre`, `french`, `jpn`, `japanese`, ...). You can also set the default with `export ANI_CLI_DUB_LANG=fre,eng`. A `change_language` option is also available in the playback menu to pick from the languages actually available for the current episode.
* Can I watch in VF/VOSTFR? - Yes! Use `--site anime-sama` or `--site franime` to switch source. On these sites playback is French-first: it plays VF by default and automatically falls back to VOSTFR when an episode has no VF (force VOSTFR with `ANI_CLI_MODE=sub`). A `change_language` option in the playback menu lets you switch between them on the fly. The active anime-sama domain is resolved automatically through anime-sama.pw's official domain list.
